...
module tb_l2_axi_master_vip_seq2;
  ...
  logic [511:0] wr_line0, wr_line1, rd_line;
  logic [3:0]   got_id;
  logic [1:0]   got_resp;
  ...
  initial begin
    ...
    wr_line1 = mk_line(32'h2222_0000);
    ...
    // Two back-to-back writes using the same ID, to check request queuing and write sequencing.
    drive_req(1'b1, 4'h0, 32'h0000_1000, wr_line0);
    drive_req(1'b1, 4'h0, 32'h0000_2000, wr_line1);

    wait_wresp(got_id, got_resp);
    check_okay("WRITE0", got_resp);
    if (got_id !== 4'h0) begin
      $display("[TB] ERROR: expected write response ID 0, got %0d", got_id);
      $fatal;
    end

    wait_wresp(got_id, got_resp);
    check_okay("WRITE1", got_resp);
    if (got_id !== 4'h0) begin
      $display("[TB] ERROR: expected write response ID 0, got %0d", got_id);
      $fatal;
    end

    // Then two reads back-to-back from the same ID and verify both lines.
    drive_req(1'b0, 4'h0, 32'h0000_1000, '0);
    drive_req(1'b0, 4'h0, 32'h0000_2000, '0);

    wait_rresp(got_id, rd_line, got_resp);
    check_okay("READ0", got_resp);
    if (got_id !== 4'h0) begin
      $display("[TB] ERROR: expected read response ID 0, got %0d", got_id);
      $fatal;
    end
    if (rd_line !== wr_line0) begin
      $display("[TB] ERROR: readback data mismatch for addr 0x00001000");
      $fatal;
    end

    wait_rresp(got_id, rd_line, got_resp);
    check_okay("READ1", got_resp);
    if (got_id !== 4'h0) begin
      $display("[TB] ERROR: expected read response ID 0, got %0d", got_id);
      $fatal;
    end
    if (rd_line !== wr_line1) begin
      $display("[TB] ERROR: readback data mismatch for addr 0x00002000");
      $fatal;
    end

    $display("[TB] PASS: two queued writes and two queued reads completed correctly through AXI VIP memory.");
    $finish;
  end
