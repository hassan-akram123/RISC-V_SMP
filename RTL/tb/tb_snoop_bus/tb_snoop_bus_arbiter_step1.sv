// tb_snoop_bus_arbiter_step1.sv
// Minimal compile + smoke test for Step 1 port contract.
`timescale 1ns/1ps

module tb_snoop_bus_arbiter_step1;
    ...
    snoop_bus_arbiter dut (
      ...
      initial begin
        $dumpfile("dump.vcd");
        $dumpvars(0, tb_snoop_bus_arbiter_step1);
     end
    // 100MHz clock
    initial clk = 1'b0;
    always #5 clk = ~clk;

    // Simple reset + stimulus
    initial begin
        // Defaults
        rst_n = 1'b0;
        ...
        // Try to "request" from multiple masters (should not be accepted in Step 1)
        @(posedge clk);
        i0_req_valid <= 1;
        i0_req_cmd   <= 3'b000;
        i0_req_addr  <= 32'h8000_0000;
        i0_req_src   <= 2'b00;

        d1_req_valid <= 1;
        d1_req_cmd   <= 3'b001;
        d1_req_addr  <= 32'h9000_0000;
        d1_req_src   <= 2'b01;
        ...
        $display("TB done. If you saw no assertion failures, Step1 wiring contract compiles + runs.");
        $finish;
    end

    // Assertions: in Step 1 everything should remain idle/tied-off
    always @(posedge clk) begin
        if (rst_n) begin
            assert(i0_req_ready == 0) else $fatal("Step1: i0_req_ready should be 0");
            assert(i1_req_ready == 0) else $fatal("Step1: i1_req_ready should be 0");
            assert(d0_req_ready == 0) else $fatal("Step1: d0_req_ready should be 0");
            assert(d1_req_ready == 0) else $fatal("Step1: d1_req_ready should be 0");

            assert(bus_req_valid == 0) else $fatal("Step1: bus_req_valid should be 0");
            assert(bus_dat_valid == 0) else $fatal("Step1: bus_dat_valid should be 0");
            assert(i_bus_gnt_valid == 0) else $fatal("Step1: i_bus_gnt_valid should be 0");
            assert(d_bus_gnt_valid == 0) else $fatal("Step1: d_bus_gnt_valid should be 0");

            assert(d0_sup_ready == 0) else $fatal("Step1: d0_sup_ready should be 0");
            assert(d1_sup_ready == 0) else $fatal("Step1: d1_sup_ready should be 0");
            assert(l2_sup_ready == 0) else $fatal("Step1: l2_sup_ready should be 0");
        end
    end
